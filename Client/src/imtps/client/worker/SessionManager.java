package imtps.client.worker;

import imtps.client.datapacket.ControlPacket;
...
import imtps.client.session.ImtpsSession;
import imtps.client.session.channel.AcceptChannel;
import imtps.client.session.channel.DataChannel;
...
public class SessionManager extends Thread {
    ...
    public SessionManager(PacketHandler packetHandler, ProcessingHub processingHub, ImtpsLogger imtpsLogger) throws IOException {
            ...
            try {
                ...
                if (type == ImtpsChannel.TYPE.Control) {
                    ...
                    if (dataPacket != null) {
                        ...
                        processingHub.handleDataPacket(dataPacket);
                    } else {
                        closeChannel(selectionKey);
                    }
                }
            ...
            synchronized (lock) {
                if (dataChannel.getStatus() == DataChannel.STATUS.Unconnected) {
                    switch (dataPacket.getDataBodyType()) {
                        case Basic -> putControlPacket(new ControlPacket(ControlPacket.WAY.ADDRESS_DATA_BASIC));
                        case File -> putControlPacket(new ControlPacket(ControlPacket.WAY.ADDRESS_DATA_FILE));
                    }
    ...
    protected ImtpsSession getImtpsSession() {
        return imtpsSession;
    }
    ...
    private void handleControlPacket(ControlPacket controlPacket) throws Exception {
        switch (controlPacket.getWay()) {
            case ControlPacket.WAY.ADDRESS_DATA_BASIC -> {
                String address = new String(controlPacket.getContent(), StandardCharsets.UTF_8);
                int lastColon = address.lastIndexOf(':');
                sessionHandshaker.transmit(new AcceptChannel(SocketChannel.open(new InetSocketAddress
                        (address.substring(0, lastColon), Integer.parseInt(address.substring(lastColon + 1)))), ImtpsChannel.TYPE.DataBasic));
            }
            case ControlPacket.WAY.ADDRESS_DATA_FILE -> {
                String address = new String(controlPacket.getContent(), StandardCharsets.UTF_8);
                int lastColon = address.lastIndexOf(':');
                sessionHandshaker.transmit(new AcceptChannel(SocketChannel.open(new InetSocketAddress
                        (address.substring(0, lastColon), Integer.parseInt(address.substring(lastColon + 1)))), ImtpsChannel.TYPE.DataFile));
            }
