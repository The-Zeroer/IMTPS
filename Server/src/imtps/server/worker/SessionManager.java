...
import imtps.server.datapacket.ControlPacket;
...
public class SessionManager extends Thread {
    ...
    public void stopRunning() {
        ...
        System.out.println("Manager stopped: " + atomicInteger.get());
    }
    ...
    private void readEvent(SelectionKey selectionKey) {
        ...
        threadPool.submit(() -> {
            try {
                ControlPacket controlPacket = packetHandler.readControlPacket(selectionKey, imtpsSession.getImtpsSecretKey());
                if (controlPacket != null) {
                    imtpsLogger.trace("接收ControlPacket[$]", controlPacket);
                    handleControlPacket(imtpsSession, controlPacket);
                } else {
                    closeChannel(imtpsSession, ImtpsChannel.TYPE.Control);
                }
            } catch (Exception e) {
    ...
    private void handleControlPacket(ImtpsSession imtpsSession, ControlPacket controlPacket) throws InterruptedException {
        switch (controlPacket.getWay()) {
            ...
            case ControlPacket.WAY.ADDRESS_DATA_BASIC -> putControlPacket(imtpsSession.getControlChannel().getSelectionKey(),
                    new ControlPacket(ControlPacket.WAY.ADDRESS_DATA_BASIC, addressManager.choose
                            (imtpsSession, ImtpsChannel.TYPE.DataBasic).getBytes(StandardCharsets.UTF_8)));
            case ControlPacket.WAY.ADDRESS_DATA_FILE -> putControlPacket(imtpsSession.getControlChannel().getSelectionKey(),
                    new ControlPacket(ControlPacket.WAY.ADDRESS_DATA_FILE, addressManager.choose
                            (imtpsSession, ImtpsChannel.TYPE.DataFile).getBytes(StandardCharsets.UTF_8)));
        }
    }
    ...
    private void closeChannel(ImtpsSession imtpsSession, ImtpsChannel.TYPE type) {
        try {
            ...
            imtpsLogger.log(ImtpsLogger.LEVEL_DEBUG, "关闭通道[$][$]", type, imtpsSession.getChannel(type).getSocketAddress());
        } catch (Exception e) {
            imtpsLogger.log(ImtpsLogger.LEVEL_DEBUG, "关闭通道[$][$]出错", type, imtpsSession.getChannel(type).getSocketAddress(), e);}
    }
    ...
    class SelectorWorker extends Thread {
        ...
        private void readEvent(SelectionKey selectionKey) {
            ...
            threadPool.submit(() -> {
                try {
                    DataPacket dataPacket = packetHandler.readDataPacket(selectionKey, imtpsSession.getImtpsSecretKey());
                    if (dataPacket != null) {
                        imtpsLogger.trace("接收DataPacket[$]", dataPacket);
                        processingHub.handleDataPacket(dataPacket, (ImtpsSession) selectionKey.attachment());
                    } else {
                        closeChannel(imtpsSession, type);
                    }
                } catch (Exception e) {
            ...
            threadPool.submit(() -> {
                try {
                    ...
                    while (!dataPacketQueue.isEmpty()) {
                        packetHandler.writeDataPacket(selectionKey, imtpsSession.getImtpsSecretKey(), Objects.requireNonNull(dataPacketQueue.poll()));
                    }
                } catch (Exception e) {
        ...
        public void addWriteEvent(ImtpsSession imtpsSession) {
            if (imtpsSession.getDataChannel(type).getStatus() == ImtpsChannel.STATUS.Unconnected) {
                switch (type) {
                    case ImtpsChannel.TYPE.DataBasic -> putControlPacket(imtpsSession.getControlChannel().getSelectionKey(), new ControlPacket
                            (ControlPacket.WAY.ADDRESS_DATA_BASIC, addressManager.choose(imtpsSession, ImtpsChannel.TYPE.DataBasic).getBytes(StandardCharsets.UTF_8)));
                    case ImtpsChannel.TYPE.DataFile -> putControlPacket(imtpsSession.getControlChannel().getSelectionKey(), new ControlPacket
                            (ControlPacket.WAY.ADDRESS_DATA_FILE, addressManager.choose(imtpsSession, ImtpsChannel.TYPE.DataFile).getBytes(StandardCharsets.UTF_8)));
                }
            } else {
            ...
            public void run() {
                ...
                for (ImtpsSession imtpsSession : idToSessionHashMap.values()) {
                    ...
                    if (imtpsChannel.getStatus() == ImtpsChannel.STATUS.Connected && imtpsChannel.getLastActivityTime() + interval < nowTime
                        ...
                        imtpsLogger.log(ImtpsLogger.LEVEL_DEBUG, "通道心跳超时[$][$]" , type, imtpsSession.getSessionName());
                        closeChannel(imtpsSession, type);
                    }
                }
            }
